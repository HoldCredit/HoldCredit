...
# 항목과 타겟 변수를 분리
# 뒤에서 3번째 CB등급 항목임.
X = df[df.columns[:-3]]  # 마지막 3개 컬럼 전까지가 항목
y = df[df.columns[-3]]   # 마지막 3번째 컬럼이 타겟 변수
...
# 중요도가 0.01 이상인 항목를 선택 (threshold=0.01), 전체 항목(threshold=0)
...
# 항목 추출 -> 엑셀파일
important_features.to_excel('신용등급 산출의 주요한 컬럼 추출.xlsx', index=False)